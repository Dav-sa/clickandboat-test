...
class AutomaticEmailManager
{
    ...
    {
        $fallbackLanguage = Language::getInstance()->filterCode(
            $this->languages,
            "en"
        );

        if (isset($this->data["language_id"])) {
            return Language::getInstance()->filterId(
                $this->languages,
                $this->data["language_id"],
                true
            );
        } elseif (isset($this->data["recipient_account_id"])) {
            return Language::getInstance()->byAccountId(
                $this->data["recipient_account_id"]
            );
        } else {
    ...
    {
        return (!isset($template->isEnabled) || !$template->isEnabled) &&
            !$this->isForPreview;
    }
    ...
    private function replaceProductVars($template)
    {
        if (isset($this->data["product"])) {
            ...
            $this->replaceVar(
                $prefix . "_city",
                $this->data["product"]->city,
                $template->content
            );
            $this->replaceVar(
                $prefix . "_model",
                $this->data["product"]->model,
                $template->content
            );
            $this->replaceVar(
                $prefix . "_builder",
                $this->data["product"]->builder,
                $template->content
            );
        }
    }
            ...
            if ($this->params["signature"] === true) {
                if (isset($data["sender"]) && null !== $language) {
                    $this->params[
                        ...
                        $language,
                        "DefaultAutomaticEmailSignature"
                    );
                } else {
